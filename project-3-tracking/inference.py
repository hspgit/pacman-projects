...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    # All grid positions (ignore walls as requested)
    all_positions = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]

    # Maximum possible true Manhattan distance on the grid (corner to corner)
    max_true_distance = (X_RANGE - 1) + (Y_RANGE - 1)

    # Observation domain: non-negative integers up to max_true_distance + MAX_NOISE
    obs_domain = list(range(0, max_true_distance + MAX_NOISE + 1))
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # Get all CPTs with evidence applied
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        
        # Iterate over variables in elimination order
        for eliminationVariable in eliminationOrder:
            ...
            # If so, just discard it (don't eliminate and don't add back)
            if len(joinedFactor.unconditionedVariables()) == 1:
                # Just discard this factor - don't add it back to currentFactorsList
                continue
            
            # Otherwise, eliminate the variable from the joined factor
            eliminatedFactor = eliminate(joinedFactor, eliminationVariable)
            
            # Add the result back to the list of factors
            currentFactorsList.append(eliminatedFactor)
        
        # Join any remaining factors to get the final result
        if len(currentFactorsList) > 0:
            finalFactor = joinFactors(currentFactorsList)
        else:
            # If no factors remain, create a trivial factor
            finalFactor = currentFactorsList[0] if currentFactorsList else None
        
        # Normalize the final factor to get a proper conditional probability
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        # If total is 0 or distribution is empty, do nothing
        if total == 0:
            return
    ...
    def sample(self):
        ...
        # Find which key this corresponds to using cumulative sum
        cumulative = 0.0
        for key in self.keys():
            cumulative += self[key]
            if rand <= cumulative:
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        # Normal case: ghost is not in jail and observation is not None
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # Update belief for each position using Bayes' rule
        # P(position | observation) ∝ P(observation | position) × P(position)
        for position in self.allPositions:
            # Get P(observation | position) using the method we implemented
            observationProb = self.getObservationProb(observation, pacmanPosition, position, jailPosition)
            
            # Update belief: P(position | observation) = P(observation | position) × P(position)
            self.beliefs[position] = self.beliefs[position] * observationProb
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        # Create a new distribution for beliefs at time t+1
        newBeliefs = DiscreteDistribution()
        
        # For each possible new position at time t+1
        for newPos in self.allPositions:
            # Sum over all possible old positions at time t
            # P(newPos at t+1) = sum over oldPos of P(newPos | oldPos) * P(oldPos at t)
            
            probability = 0.0
            
            for oldPos in self.allPositions:
                # Get the transition probability distribution from oldPos
                # This gives us P(newPos | oldPos, gameState)
                newPosDist = self.getPositionDistribution(gameState, oldPos)
                
                # P(newPos at t+1) += P(newPos | oldPos) * P(oldPos at t)
                probability += newPosDist[newPos] * self.beliefs[oldPos]
            
            # Set the accumulated probability for this new position
            newBeliefs[newPos] = probability
        
        # Normalize the new belief distribution
        newBeliefs.normalize()
        
        # Update our beliefs to the new distribution
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        # Use mod operator to cycle through positions
        for i in range(self.numParticles):
            # Use mod to ensure even distribution
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # Create a distribution to hold weights for each particle position
        beliefs = DiscreteDistribution()
        
        # Reweight each particle based on observation probability
        for particle in self.particles:
            # Get P(observation | particle position)
            weight = self.getObservationProb(observation, pacmanPosition, particle, jailPosition)
            # Add weight to this position
            beliefs[particle] += weight
        
        # Special case: if all particles have weight 0, reinitialize
        if beliefs.total() == 0:
            self.initializeUniformly(gameState)
            return
        ...
        newParticles = []
        for i in range(self.numParticles):
            # Sample a position according to the weighted distribution
    ...
    def elapseTime(self, gameState):
        ...
        gameState.
        """
        # Create a new list for updated particles
        newParticles = []
        
        # For each current particle
        for oldParticle in self.particles:
            # Get the distribution over next positions given the current position
            # This gives us P(newPos | oldPos, gameState)
            ...
            # Add the new position to our new particles list
            newParticles.append(newParticle)
        
        # Update particles with the new particles
        self.particles = newParticles
